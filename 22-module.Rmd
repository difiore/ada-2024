...
# Model Selection in Linear Regression {#module-22}
...
```{r}
anova(m2, m1, test = "F")
# compares the reduced model without interactions (m2) to the full model
# with interactions (m1)
```
...
summary(m0)
add1(m0, scope = . ~ . + age + weight + zombies_killed +
  years_of_education, test = "F")
m1 <- update(m0, formula = . ~ . + weight)
summary(m1)
add1(m1, scope = . ~ .++age + weight + zombies_killed +
  years_of_education, test = "F")
m2 <- update(m1, formula = . ~ . + age)
summary(m2)
add1(m2, scope = . ~ . + age + weight + zombies_killed +
  years_of_education, test = "F")
```
...
Note that the `stepAIC()` function has converged (using AIC, in this case) on the same best model we found above through forward and backwards selection by hand - i.e., the model with just age and weight as predictor variables that we retain after our process of model selection.
...
What makes this wrapper especially useful is that the output very clearly compares AIC values for different models in a way often asked for in publications. Let's take a look using our already-defined models.
...
```{r}
library(MuMIn)
# start by defining a "global.model"
m <- lm(data = z,
  height ~ age + weight + zombies_killed + years_of_education,
  na.action = "na.fail")
# here, we need to change the "na.action" argument from the
#   default of "na.omit" to "na.fail" to prevent `dredge()` from
#   trying to fit submodels with different data sets than the
#   global model if there are missing values
# alternatively, we could set that option globally for our R
#   session before running the global model by using
#   `options(na.action = na.fail)` and then omitting the
#   `na.action=` argument from our call to `lm()`
# options()$na.action shows the current na.action option

# we then run `dredge()` to explore a set of submodels
#   the `m.lim=` argument is optional and sets lower and upper
#   limits on the number of terms to include
#   the `beta=` argument specifies whether or not and how to
#   standardize the beta coefficients; "none" is the default
m.set <- dredge(m, beta = "none", m.lim = c(0, 4))
(res <- get.models(m.set, subset=TRUE))
coef(m.set) # returns a table of coefficients from each model
```

 Averaging coefficients across sets of models...
 
```{r}
# summarizing models with delta.aicc < 4
#   here, we need to set the argument `fit=TRUE` because
#   `dredge()` does not store the model results
(m.avg <- summary(model.avg(m.set,
  subset = delta < 4, fit = TRUE)))
# and getting confidence intervals for the average coefficient
#   estimates across models...
confint(m.avg)

# NOTE: The ‘subset’ (or ‘conditional’) average only averages
#   over models where the term appears. The ‘full’ average assumes
#   that a term is included in every model but the estimate and
#   standard error are set to zero in models where the term is not
#   included

# summarizing models as a 95% confidence set, i.e., for cumulative
#   AIC weights up to 95%

(m.avg <- summary(model.avg(m.set, subset =
  cumsum(weight) <= .95, fit = TRUE)))
confint(m.avg)
detach(package:MuMIn)
```

Note that, again, the two key predictors of zombie apocalypse survivor's **height** are **age** and **weight**, and the best models include both of these terms!
