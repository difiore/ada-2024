...
# extract coordinates from the original data frame
coords <- d |> dplyr::select(Longitude, Latitude)
# specify the crs of original data
proj_original <- "+proj=longlat +datum=WGS84"
# create an `sp` object
d_sp <- SpatialPointsDataFrame(coords = coords, data = d, proj4string = CRS(proj_original))

# we can then call `proj4string()` to assign the projection to our SpatialPointsDataFrame
...
# d_sp has class `sp` and `SpatialPointsDataFrame`
plot(d_sp, pch = 1, main = "Focal Points")

locs <- SpatialPointsDataFrame(coords = c(d$Longitude, d$Latitude), data = d, proj4string = crs_target)
...
station_sf <- st_as_sf(station_sp, CRS(crs_target))
```
...
rio_sf <- st_as_sf(rio_sp, CRS(crs_target))
```
