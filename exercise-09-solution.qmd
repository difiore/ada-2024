...
(SE_b1 <- sqrt(sum(residuals^2)/(n-2)/sum((d_mod$Group_size - mean(d_mod$Group_size))^2)))
...
(p.percentile <- perm |> 
  ...
  pull(p))
# p value taken directly from the permutation distribution
# i.e., how many of the slope estimates from permuted
# samples are more extreme than the actual b1?
...
# the t statistic used in `pt()` to calculate the p values is
# b1/se_b1, 
# where se_b1 is estimated as the standard deviation
# of the permutation 
# distribution (i.e., 1/perm_se)
# the `2 *` the upper-tail probability in this calculation
# is to make this a 2-tailed test
```
...
```{r}
# using a loop to get a bootstrap distribution to generate
# a CI for the slope coefficient
nboot <- 1000
...
# note... the CI_theory values calculated with `get_ci()`
# differ very slightly from those calculated by hand
# because {infer} seems to be using upper/lower
# 0.025% boundaries of a *normal* rather than a *t* distribution
# I believe this is an "error" in how `get_ci()` is implemented
# in {infer}
