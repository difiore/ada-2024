...
The top level of your repo should include an ".Rmd" or ".qmd" file called "data-analysis-replication.Rmd" where you thoroughly describe and run the code for all of the steps in your reanalysis. You can begin with the standard **RMarkdown** or **Quarto** document template created by choosing **File \> New File** and the desired file type and then choosing `HTML` as the "Default Output Format".
...
Within the repo, also make an "images" directory in which in include ".jpeg" or ".png" or similar files that show figures or tabular results from the original paper. In your ".Rmd" file, near your own results, you will want to embed some of these results from the original paper that you replicate so that I can see them together. You can include code in one of the following formats to reference files in your "images" folder for inclusion in your document. The first alternative uses raw HTML, the second and third accomplish the same thing using {knitr} in an {r} code block in either an **RMarkdown** (Alternative 2) or **Quarto** (Alternative 3) document.
...
In each case, you would replace *imagename.filetype* with the name of your file, e.g., "figure-1.jpeg" and *\#\#\#* with a integer number of pixels (e.g., 200px) or a percent of the window width (e.g., "70%").

Also add the following line to the initial code chunk in your ".Rmd" or ".qmd" file, which tells {knitr} where to output all of the figures associated with your chunks of code as well as where to find any images that you want to include in your ".html" output.


`knitr::opts_chunk$set(fig.path = "images/")`

If you wish, you can download either the file "data-analysis-replication-template.Rmd" or "data-analysis-replication-template.qmd" from <https://github.com/difiore/ada-2024-datasets> and use that as a starting point for your file.

Before turning in this assignment, you should confirm that you can render your document successfully to HTML and make sure that the entire repo is pushed to ***GitHub*** repository as well. **I should be able to clone your entire repo to my own computer, open the ".Rmd" or ".qmd" file, and render it to produce a nicely formatted ".html" report describing what you did and seeing your results.**
