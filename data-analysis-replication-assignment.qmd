...
# (PART\*) Assignments {.unnumbered}

# Data Analysis Replication Assignment {.unnumbered}
...
You do not need to replicate ALL of the analysis presented in the paper (although the more the better)! At a bare minimum, you need to repeat at least *three* analyses, including at least one descriptive statistical analysis, one visualization, and one inferential statistical analysis.
...
-   Organization and Logistics
    ...
    -   ".Rmd" file for your report stored at the top level of the repo
    -   ".Rmd" file includes text and code and embedded images/tables from the paper for comparison
    -   ".Rmd" file well organized into subsections, including blocks of explanatory text and ***R*** code blocks, plus output
...
-   Introduction and Framing
    ...
    -   ".Rmd" file knits and produces HTML output without requiring edits or additional modifications
    -   ".Rmd" file defines variable names used in ***R*** code
...
The top level of your repo should include an ".Rmd" file called "data-analysis-replication.Rmd" where you thoroughly describe and run the code for all of the steps in your reanalysis. You can begin with the standard **RMarkdown** document template created by choosing **File \> New File \> R Markdown** and choosing `HTML` as the "Default Output Format".
...
Within the repo, also make an "images" directory in which in include ".jpeg" or ".png" or similar files that show figures or tabular results from the original paper. In your ".Rmd" file, near your own results, you will want to embed some of these results from the original paper that you replicate so that I can see them together. You can include code in either of the following formats to reference files in your "images" folder for inclusion in your document. The first alternative uses raw HTML, the second accomplishes the same thing block using {knitr} in an {r} code block.
...
In each case, you replace *imagename.filetype* with the name of your file, e.g., "figure-1.jpeg" and *\#\#\#* with a integer number of pixels (e.g., width="200px") or a percent of the window width (e.g., out.width="70%").

Also add the following line to the initial code chunk in your ".Rmd" file, which tells {knitr} where to output all of the figures associated with your chunks of code as well as where to find any images that you want to include in your ".html" output.

```
knitr::opts_chunk$set(fig.path = "images/")
```

If you wish, you can download the file "data-analysis-replication-template.Rmd" from <https://github.com/difiore/ada-2022-datasets> and use that as a starting point for your file.

Before turning in this assignment, you should confirm that you can "knit" your document successfully to HTML and make sure that the entire repo is pushed to ***GitHub*** repository as well. **I should be able to clone your entire repo to my own computer, open the ".Rmd" file, and knit it to produce a nicely formatted ".html" report describing what you did and seeing your results.**
