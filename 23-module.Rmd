...
::: {.fold .s .o}
```{r}
f <- "https://raw.githubusercontent.com/difiore/ada-2022-datasets/main/graddata.csv"
...
Alternatively, we can use the `confint.default()` function, whcih returns a CI based on standard errors, the way we have calculated them by hand previously.
...
A likelihood ratio test comparing a proposed and reduced model can be performed using the `anova()` function with the additional argument `test="Chisq"` rather than `test="F"`.
...
The $\chi^2$ value is then just the difference of the deviances...
