...
-   Install this package in ***R***: [{jtools}](https://cran.r-project.org/web/packages/jtools/jtools.pdf)
...
```{r}
ms <- apply(original, 2, FUN = "mean") # returns a vector of means, where we are taking this across dimension 2 of the array "orig"
ms

# or
ms <- unlist(summarize(original, ymean = mean(Y), x1mean=mean(X1),x2mean=mean(X2),x3mean=mean(X3)))
ms

sds <- apply(original, 2, FUN = "sd")
sds

# or
sds <- unlist(summarize(original, ySD = sd(Y), x1SD=sd(X1),x2SD=sd(X2),x3SD=sd(X3)))
sds

normalized <- sweep(original,2,STATS=ms,FUN="-") # 2nd dimension is columns, removing array of means, function = subtract
...
head(df)
cor(df)
plot(df) # note the change to the axis scales
# using `pairs(d)` would produce the same plot
```

We now have our own dataframe, **df**, comprising correlated random variables in original units!

Let's explore this dataset, first with single and then with multivariate regression.

### CHALLENGE: {.unnumbered}

Start off by making three bivariate scatterplots in {ggplot2} using the dataframe of three predictor variables (**X1**, **X2**, and **X3**) and one response variable (**Y**) that we generated above (or download it from f \<- "<https://raw.githubusercontent.com/difiore/ada-2022-datasets/main/multiple_regression.csv>")

```{r}
f <- "https://raw.githubusercontent.com/difiore/ada-2022-datasets/main/multiple_regression.csv"
df <- read_csv(f, col_names = TRUE)
```

::: {.fold .s .o}
```{r}
g1 <- ggplot(data=df, aes(x = X1, y = Y)) + geom_point() +
  geom_smooth(method="lm", formula=y~x)
g2 <- ggplot(data=df, aes(x = X2, y = Y)) + geom_point() +
  geom_smooth(method="lm", formula=y~x)
g3 <- ggplot(data=df, aes(x = X3, y = Y)) + geom_point() +
  geom_smooth(method="lm", formula=y~x)
...
Load up the "zombies.csv" dataset with the characteristics of zombie apocalypse survivors again and run a linear model of height as a function of both weight and age. Is the overall model significant? Are both predictor variables significant when the other is controlled for?
...
```{r}
w <- lm(weight ~ gender + age, data = z)
rsq_w <- summary(w)$r.square
(vif_w <- 1/(1-rsq_w))

a <- lm(age ~ gender + weight, data = z)
...
g <- lm(as.numeric(gender) ~ age + weight, data = z)
...
p <- ggplot(data = z %>% filter(gender == "Male"),
  aes(x = age, y = height)) +
  geom_point() +
...
::: {.fold .s .o}
```{r}
m <- lm(data = z, height ~ age + gender + age:gender)
summary(m)
# or
...
-   Reduce the dataset to the following variables: **Family**, **Brain_Size_Female_Mean**, **Body_mass_female_mean**, **MeanGroupSize**, **DayLength_km**, **HomeRange_km2**, and **Move** and assign **Family** to be a factor.
...
head(d)
d <- select(d, Brain_Size_Female_Mean, Family, Body_mass_female_mean,
  MeanGroupSize, DayLength_km, HomeRange_km2, Move)
```
:::
...
m <- lm(data = d, log(HomeRange_km2) ~ log(Body_mass_female_mean) +
  log(Brain_Size_Female_Mean) + MeanGroupSize + Move)
summary(m)
plot(m$residuals)
qqnorm(m$residuals)
...
```{r}
m <- lm(data=d, log(HomeRange_km2)~log(Body_mass_female_mean) +
  log(Brain_Size_Female_Mean)+ MeanGroupSize)
summary(m)
plot(m$residuals)
qqnorm(m$residuals)
