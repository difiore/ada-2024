...
- Using the {tidyverse} `read_csv()` function, load the "KamilarAndCooperData.csv" dataset as a "tibble" named **d**.
...
The p value for the Shapiro-Wilk test is less than 0.05 for both the m1 and m2 residuals, leading us to **reject** the null hypothesis that that they are normally distributed (or, better put, that they do not deviate more than expected from normality). The p value for the test of the m3 residuals is >> 0.05, leading us to **not reject** the null hypothesis that that they are normally distributed. 
