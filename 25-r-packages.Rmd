---
title: "Custom ***R*** Package Development"
---

# (PART\*) Part III - Miscellany {.unnumbered}
...
Because I know that the function I just created uses functions from {ggplot2}, I want to add a reference to that package as a dependency to my DESCRIPTION file. To do this, I can run the following:
...
You can also install your package to your system or user library by using the {devtools} function `install()`.
...
# this puts my new {powerplot} package into my system library instead of
# my user library... by default, it installs in the user library and that
# is okay, but I just like to keep all my libraries in one place
detach(package:withr)
```

You can also use the {base} function `install.packages()` function with the path to the ".tar.gz" file you created with `build()` as an argument. Here, we use `repos=NULL` because we are installing from a local file and we set `type=source` as we're installing a bundled, compressed source file. In the code below, I have set `lib=` to be the location of my system library, but you can instead use a user library. You may need to change this path depending on where your system library is located.
...
The resulting knitted ".html" version of your vignette will be placed into the `inst/doc` folder inside the package directory.
...
# this puts powerplot in my system library instead of my user library...
# by default, it installs in the user library and that is okay,
# I just like to keep all my libraries in one place
detach(package:withr)
# OR
install.packages("~/Development/repos/powerplot_0.0.1.0.tar.gz",
  repos = NULL, type = "source",
...
Just open the file, edit, and save!
...
```{r message=FALSE, fig.align = "center", out.width="50%", dpi=300, eval=FALSE}
library(car)
library(ggplot2)
library(jtools)
library(sjPlot)
