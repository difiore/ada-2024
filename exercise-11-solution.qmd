...
Given these deviations from the assumptions for simple linear regression, we are not *really* justified in performing the analyses done above using standard linear modeling `lm()` or ANOVA `aov()`.
