...
my_data <-
  data.frame(
    "name" = c("Ned", "Sansa", "Cersei",  "Tyrion", "Jon", "Daenerys",
               "Aria", "Brienne", "Rickon", "Edmure", "Petyr", "Jamie",
               "Robert", "Stannis", "Theon"
    ),
    "house" = c("Stark", "Stark", "Lannister", "Lannister", "Stark",
                "Targaryen", "Stark", "Tarth", "Stark", "Tully", 
                "Baelish", "Lannister", "Baratheon", "Baratheon", "Greyjoy"
    ),
    "code" = sample(100000:999999, 15, replace = FALSE)
  )
my_data <- add_prefix(my_data, variable = "house") # uses default prefix
head(df)
my_data <- add_prefix(my_data, prefix = "House ", variable = "house") 
head(df)
my_data <- add_prefix(my_data, prefix = "00001-", variable = "code") 
head(my_data)
```
...
Write a `for()` loop to print out each row in the data frame **df**.
...
Write a `for()` loop to print out the reverse of each element in the **code** vector in the data frame **df**.
...
As above, the following example print out each element in the vector, **v**:
...
# Here we are passing on an extra argument to the `median()`function,
# i.e., `na.rm=TRUE`
# This is an example of "dot-dot-dot" (`...`) being an extra argument
# of the `sapply()` function where those arguments are "passed through"
# as arguments of the `FUN=` function
# Basically, this means that we can pass on an arbitrary set and number
# of arguments into `sapply()` which, in this case, are then being used
