...
> **NOTE:** For both of the approaches below, before using Google's gmail server to send emails, a key Gmail account setting that has to be changed from the default value: you have to have 2-factor authorization established for your account and you will need to set up an "app password". We can go over this process in class.
...
smtp_gmail <- server(
  ...
  password = "") # add your Google account app password between quotation marks here

smtp_outlook365 <- server(
  host = "smtp.office365.com",
  port = 587,
  username = "anthony.difiore@austin.utexas.edu",
  password = "")  # add your UT email password between quotation marks here
```
