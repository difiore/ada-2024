...
If you have a Google **Gmail** account (either a personal one or one associated with UT, e.g., a <username>@utexas.edu account) with 2-factor authorization established, you can follow the steps below to create an "app password" that you can use for authentication.
...
> **NOTE:** `from=` should typically be the email address you are sending the from, although some SMTP transactions will accept alternatives, such as just a name. You can also use a vector of email addresses for the `to=` argument to send a message to more than one recipient.
...
> **NOTE:** `max_times=` sets the number of attempts the function will make to send the message. The default (if the argument is omitted) is 5, and below I have set it to 1 to exit quickly if the server is not contacted. For `username=` enter your login ID for the service you are sending from, and for `password=` use the variable you set up above to hold your "app password" for your Google account, or enter your password directly here, in quotation marks it, in lieu of the variable `gmail_pwd`
...
# either use a variable, e.g., `outlook_pwd` for your password or add your password between quotation marks
```
...
## Version 2: Using the {mailR} package {.unnumbered}
...
- Using Microsoft's Outlook/Office365 server...
    - To use the Outlook/Office365 server, `port=` should be set to 587 and the argument `tls=` (and not `ssl=`) must be set to TRUE to allow TLS encryption.
...
## Version 3: Using the {blastula} package {.unnumbered}

The {blastula} package allows us to create and send more complex HTML and markdown-based formatted emails. Using it is a bit more complicated (but much more flexible) than using the procedures above.
...
# Preview the message. This will open the HTML message in a browser window or in the RStudio Viewer tab
message
```
...
# credentials file for an Outlook account
create_smtp_creds_file(
...
# send using Microsofts's servers, using the %>% operator and different credentials file...
message %>% smtp_send(
    from = "anthony.difiore@outlook.com",
    to = "anthony.difiore@gmail.com",
    subject = "Sending a message using {blastula} and a credentials file",
...
```{r eval=FALSE}
# create Gmail key with a user-specified name, `id=`
...
# create Outlook key with a user-specified name, `id=`
create_smtp_creds_key(
  id = "my_outlook_key",
  user = "anthony.difiore@outlook.com",
  provider = "office365",
...
# send a message with credentials from a keychain using Microsoft's servers...
message %>%
  smtp_send(
    from = "anthony.difiore@outlook.com",
    to = "anthony.difiore@gmail.com",
    subject = "Sending a message using {blastula} and and credentials from a keychain",
    credentials = creds_key(id = "my_outlook_key")
  )
```
