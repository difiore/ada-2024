...
coef(mods) # returns a table of coefficients from each model
```
...
The code below summarizes models as a 95% confidence set, i.e., for cumulative AIC weights up to 95%:

```{r}
(mods.avg <- summary(model.avg(mods, subset =
