...
We can generate a null distribution for our slope estimates via permutation using {infer} package functions... essentially, each permutation involves randomly shuffling values of the response variable under the null hypothesis that they are independent of the explanatory variable.
...
lines(x=df$age,y=df$c.lwr, col="blue")
lines(x=df$age,y=df$c.upr, col="blue")
lines(x=df$age,y=df$p.lwr, col="red")
lines(x=df$age,y=df$p.upr, col="red")
...
g1 <- g1 + geom_line(data = df, aes(x = age, y = .fitted),
  color = "black", lwd = 1)
g1 <- g1 + geom_line(data = df, aes(x = age, y = c.lwr),
  color = "blue")
g1 <- g1 + geom_line(data = df, aes(x = age, y = c.upr),
  color = "blue")
g1 <- g1 + geom_line(data = df, aes(x = age, y = p.lwr),
  color = "red")
g1 <- g1 + geom_line(data = df, aes(x = age, y = p.upr),
  color = "red")
g1
...
g2 <- ggplot() +
  ...
  # add a ribbon layer
  geom_ribbon(
    data = df, aes(x = age, ymin = c.lower, ymax = c.upr),
  ...
  geom_ribbon(
    data = df, aes(x = age, ymin = p.lower, ymax = p.upr),
    # ... with transparency set to 0.2
    alpha = 0.2, 
    # ... and fill color red
    fill = "red"
