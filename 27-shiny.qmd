...
- Also add two other output variables to the sidebar. The first will be used to display the linear model we construc...
...
Chunk 2 outputs a table of coefficients resulting from the linear model formula stored in `m()`, and it updates every time the user changes `input$response` or `input$predictors` because doing that updates the value of `m()` the output function first confirms that a valid linear model has been constructed.
...
Chunk 3 prints out the `lm()` being fitted.
...
Chunk 4 prints out a table of beta coefficients for the fitted model.
...
Chunk 5 uses {ggplot2} to graph the relationship between the variables we have selected as `input$response` and `input$predictors`.

The output function first confirms that a valid linear model has been constructed and whether there is one or more than one predictor variable.

The output plotted depends on the number and type of the predictor variables. with one predictor, the output may be either as a scatterplot with a fitted regression line (if `input$predictors` is a continuous variable) or a boxplot (if `input$predictors` is a factor variable).

With two predictors, at least one of which is a factor, the output may be scatterplots or boxplots, faceted by (one of) the factors. With two continuous predictor variables or with more than two predictor variables, no plot is created.
